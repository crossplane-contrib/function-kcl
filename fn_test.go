...
func TestRunFunctionSimple(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ExtraResourcesNamespaced": {
			reason: "The Function should pass through a single extra resource with matchName and matchNamespace",
			args: args{
				req: &fnv1.RunFunctionRequest{
					...
					Input: resource.MustStructJSON(`{
						...
						"spec": {
							"target": "Default",
							"source": "items = [{ apiVersion: \"meta.krm.kcl.dev/v1alpha1\", kind: \"ExtraResources\", requirements = { \"cool-ns-resource-matchname\" = { apiVersion: \"example.org/v1\", kind: \"CoolExtraResource\", matchNamespace: \"cool-ns-scoped-ns\", matchName: \"cool-ns-scoped-resource\" } } }]"
						}
					}`),
					Observed: &fnv1.State{
